...
__copyright__ = "Copyright 2021, Urban Mining Company"
__credits__ = ["Rebecca Iglesias-Flores"]
__license__ = "MIT"
__version__ = "0.1.0"
__maintainer__ = "Rebecca Iglesias-Flores"
__email__ = "riglesias-flores@urbanminingco.com"
__status__ = "Dev"
...
python -u code/yaskawa_parser.py \
--var_tracker_fname Urban_Mining_Company_792164-1-1_CDS_VAR_Rev.0.xlsx \
--dataDir data/ \
--outDir out/ \
...
def register_arguments():
    ...
    parser.add_argument(
        ...
        required=True,
        help="Filename of the excel variable tracker file. ex. 'Urban_Mining_Company_792164-1-1_CDS_VAR_Rev.0.xlsx'"
        )
    parser.add_argument(
        '--dataDir', type=str,
        required=True,
...
def pad_data(DAT_dict):
    ...
    padded_lengths = [3,2004,4004,6003,8004,10004,20004] + [0]
...
def run():
    ...
    for i, sheet_name in enumerate(relevant_sheets):
        ...
        # data always starts at index 7
        # <class 'pandas.core.frame.DataFrame'>
        selected_rows = df.iloc[7:,]
        print(f'confirm selected rows for sheet {sheet_name}')
        print(selected_rows.head())

        # Select rows which do not have NaN value in column 1 ('Unnamed: 1') of excel sheet
        # Columns: [CUSTOMER:, Unnamed: 1, Urban Mining Company, Unnamed: 3, Unnamed: 4]
        filtered_df = selected_rows[~selected_rows['Unnamed: 1'].isnull()]

        # select the hashtag column and the Name column
        temp_hashtag_col, name_col = filtered_df.iloc[:,0].map(str).to_list(), filtered_df.iloc[:,1].to_list()
        # pad zeros
